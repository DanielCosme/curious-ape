// Code generated by BobGen sqlite v0.49.0. DO NOT EDIT.
// This file is meant to be re-generated in place and/or deleted at any time.

package models

import (
	"context"
	"fmt"
	"io"
	"time"

	"github.com/aarondl/opt/null"
	"github.com/aarondl/opt/omit"
	"github.com/aarondl/opt/omitnull"
	"github.com/stephenafamo/bob"
	"github.com/stephenafamo/bob/dialect/sqlite"
	"github.com/stephenafamo/bob/dialect/sqlite/dialect"
	"github.com/stephenafamo/bob/dialect/sqlite/dm"
	"github.com/stephenafamo/bob/dialect/sqlite/sm"
	"github.com/stephenafamo/bob/dialect/sqlite/um"
	"github.com/stephenafamo/bob/expr"
	"github.com/stephenafamo/bob/mods"
	"github.com/stephenafamo/bob/orm"
	"github.com/stephenafamo/scan"
)

// SleepLog is an object representing the database table.
type SleepLog struct {
	ID             int64            `db:"id,pk" `
	DayID          int64            `db:"day_id" `
	StartTime      time.Time        `db:"start_time" `
	EndTime        time.Time        `db:"end_time" `
	IsMainSleep    null.Val[bool]   `db:"is_main_sleep" `
	TotalTimeInBed null.Val[int64]  `db:"total_time_in_bed" `
	TimeAsleep     null.Val[int64]  `db:"time_asleep" `
	Origin         string           `db:"origin" `
	Raw            null.Val[string] `db:"raw" `
	NOTE           null.Val[string] `db:"NOTE" `
	Title          string           `db:"title" `

	R sleepLogR `db:"-" `
}

// SleepLogSlice is an alias for a slice of pointers to SleepLog.
// This should almost always be used instead of []*SleepLog.
type SleepLogSlice []*SleepLog

// SleepLogs contains methods to work with the sleep_log table
var SleepLogs = sqlite.NewTablex[*SleepLog, SleepLogSlice, *SleepLogSetter]("", "sleep_log", buildSleepLogColumns("sleep_log"), sleepLogScanMapper)

// SleepLogsQuery is a query on the sleep_log table
type SleepLogsQuery = *sqlite.ViewQuery[*SleepLog, SleepLogSlice]

// sleepLogR is where relationships are stored.
type sleepLogR struct {
	Day *Day // fk_sleep_log_0
	// Loaded reports whether each relationship has been loaded.
	// A relationship's bool is set by Load*, Preload, ThenLoad, factory builds,
	// and to-one Attach/Insert operations. To-many Attach/Insert operations leave it unchanged.
	Loaded sleepLogRLoaded `db:"-" `
}

// sleepLogRLoaded tracks which relationships on SleepLog have been loaded.
type sleepLogRLoaded struct {
	Day bool // fk_sleep_log_0
}

func buildSleepLogColumns(tableName string) sleepLogColumns {
	columnsExpr := expr.NewColumnsExpr(
		"id", "day_id", "start_time", "end_time", "is_main_sleep", "total_time_in_bed", "time_asleep", "origin", "raw", "NOTE", "title",
	)

	if tableName != "" {
		columnsExpr = columnsExpr.WithParent(tableName)
	}

	return sleepLogColumns{
		ColumnsExpr:    columnsExpr,
		tableAlias:     tableName,
		ID:             buildSleepLogColumn(tableName, "id"),
		DayID:          buildSleepLogColumn(tableName, "day_id"),
		StartTime:      buildSleepLogColumn(tableName, "start_time"),
		EndTime:        buildSleepLogColumn(tableName, "end_time"),
		IsMainSleep:    buildSleepLogColumn(tableName, "is_main_sleep"),
		TotalTimeInBed: buildSleepLogColumn(tableName, "total_time_in_bed"),
		TimeAsleep:     buildSleepLogColumn(tableName, "time_asleep"),
		Origin:         buildSleepLogColumn(tableName, "origin"),
		Raw:            buildSleepLogColumn(tableName, "raw"),
		NOTE:           buildSleepLogColumn(tableName, "NOTE"),
		Title:          buildSleepLogColumn(tableName, "title"),
	}
}

type sleepLogColumns struct {
	expr.ColumnsExpr
	tableAlias     string
	ID             sleepLogColumn
	DayID          sleepLogColumn
	StartTime      sleepLogColumn
	EndTime        sleepLogColumn
	IsMainSleep    sleepLogColumn
	TotalTimeInBed sleepLogColumn
	TimeAsleep     sleepLogColumn
	Origin         sleepLogColumn
	Raw            sleepLogColumn
	NOTE           sleepLogColumn
	Title          sleepLogColumn
}

// Alias returns the current table alias for the columns set.
func (c sleepLogColumns) Alias() string {
	return c.tableAlias
}

// AliasedAs returns a copy of the columns set qualified by tableName.
func (sleepLogColumns) AliasedAs(tableName string) sleepLogColumns {
	return buildSleepLogColumns(tableName)
}

// Unqualified returns a copy of the columns set without table qualification.
func (c sleepLogColumns) Unqualified() sleepLogColumns {
	return buildSleepLogColumns("")
}

func buildSleepLogColumn(alias, name string) sleepLogColumn {
	return sleepLogColumn{
		Expression: sqlite.Quote(alias, name),
		alias:      alias,
		name:       name,
	}
}

type sleepLogColumn struct {
	sqlite.Expression
	alias string
	name  string
}

// Name returns the unqualified column name.
func (c sleepLogColumn) Name() string {
	return c.name
}

// ShouldOmitParens prevents automatic parenthesis wrapping in expression builders.
func (c sleepLogColumn) ShouldOmitParens() bool {
	return true
}

// SleepLogSetter is used for insert/upsert/update operations
// All values are optional, and do not have to be set
// Generated columns are not included
type SleepLogSetter struct {
	ID             omit.Val[int64]      `db:"id,pk" `
	DayID          omit.Val[int64]      `db:"day_id" `
	StartTime      omit.Val[time.Time]  `db:"start_time" `
	EndTime        omit.Val[time.Time]  `db:"end_time" `
	IsMainSleep    omitnull.Val[bool]   `db:"is_main_sleep" `
	TotalTimeInBed omitnull.Val[int64]  `db:"total_time_in_bed" `
	TimeAsleep     omitnull.Val[int64]  `db:"time_asleep" `
	Origin         omit.Val[string]     `db:"origin" `
	Raw            omitnull.Val[string] `db:"raw" `
	NOTE           omitnull.Val[string] `db:"NOTE" `
	Title          omit.Val[string]     `db:"title" `
}

func (s SleepLogSetter) SetColumns() []string {
	vals := make([]string, 0, 11)
	if s.ID.IsValue() {
		vals = append(vals, "id")
	}
	if s.DayID.IsValue() {
		vals = append(vals, "day_id")
	}
	if s.StartTime.IsValue() {
		vals = append(vals, "start_time")
	}
	if s.EndTime.IsValue() {
		vals = append(vals, "end_time")
	}
	if s.IsMainSleep.IsValue() || s.IsMainSleep.IsNull() {
		vals = append(vals, "is_main_sleep")
	}
	if s.TotalTimeInBed.IsValue() || s.TotalTimeInBed.IsNull() {
		vals = append(vals, "total_time_in_bed")
	}
	if s.TimeAsleep.IsValue() || s.TimeAsleep.IsNull() {
		vals = append(vals, "time_asleep")
	}
	if s.Origin.IsValue() {
		vals = append(vals, "origin")
	}
	if s.Raw.IsValue() || s.Raw.IsNull() {
		vals = append(vals, "raw")
	}
	if s.NOTE.IsValue() || s.NOTE.IsNull() {
		vals = append(vals, "NOTE")
	}
	if s.Title.IsValue() {
		vals = append(vals, "title")
	}
	return vals
}

func (s SleepLogSetter) Overwrite(t *SleepLog) {
	if s.ID.IsValue() {
		t.ID = s.ID.MustGet()
	}
	if s.DayID.IsValue() {
		t.DayID = s.DayID.MustGet()
	}
	if s.StartTime.IsValue() {
		t.StartTime = s.StartTime.MustGet()
	}
	if s.EndTime.IsValue() {
		t.EndTime = s.EndTime.MustGet()
	}
	if s.IsMainSleep.IsValue() || s.IsMainSleep.IsNull() {
		t.IsMainSleep = s.IsMainSleep.MustGetNull()
	}
	if s.TotalTimeInBed.IsValue() || s.TotalTimeInBed.IsNull() {
		t.TotalTimeInBed = s.TotalTimeInBed.MustGetNull()
	}
	if s.TimeAsleep.IsValue() || s.TimeAsleep.IsNull() {
		t.TimeAsleep = s.TimeAsleep.MustGetNull()
	}
	if s.Origin.IsValue() {
		t.Origin = s.Origin.MustGet()
	}
	if s.Raw.IsValue() || s.Raw.IsNull() {
		t.Raw = s.Raw.MustGetNull()
	}
	if s.NOTE.IsValue() || s.NOTE.IsNull() {
		t.NOTE = s.NOTE.MustGetNull()
	}
	if s.Title.IsValue() {
		t.Title = s.Title.MustGet()
	}
}

func (s *SleepLogSetter) Apply(q *dialect.InsertQuery) {
	q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
		return SleepLogs.BeforeInsertHooks.RunHooks(ctx, exec, s)
	})

	if len(q.TableRef.Columns) == 0 {
		q.TableRef.Columns = s.SetColumns()
		if len(q.TableRef.Columns) == 0 {
			q.TableRef.Columns = []string{"id"}
		}

	}

	vals := make([]bob.Expression, 0, len(q.TableRef.Columns))
	for _, col := range q.TableRef.Columns {
		switch col {
		case "id":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.ID.IsUnset() {
					return sqlite.Arg(nil).WriteSQL(ctx, w, d, start)
				}
				return sqlite.Arg(s.ID.MustGet()).WriteSQL(ctx, w, d, start)
			}))
		case "day_id":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.DayID.IsUnset() {
					return sqlite.Arg(nil).WriteSQL(ctx, w, d, start)
				}
				return sqlite.Arg(s.DayID.MustGet()).WriteSQL(ctx, w, d, start)
			}))
		case "start_time":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.StartTime.IsUnset() {
					return sqlite.Arg(nil).WriteSQL(ctx, w, d, start)
				}
				return sqlite.Arg(s.StartTime.MustGet()).WriteSQL(ctx, w, d, start)
			}))
		case "end_time":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.EndTime.IsUnset() {
					return sqlite.Arg(nil).WriteSQL(ctx, w, d, start)
				}
				return sqlite.Arg(s.EndTime.MustGet()).WriteSQL(ctx, w, d, start)
			}))
		case "is_main_sleep":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.IsMainSleep.IsUnset() {
					return sqlite.Arg(nil).WriteSQL(ctx, w, d, start)
				}
				return sqlite.Arg(s.IsMainSleep.MustGetNull()).WriteSQL(ctx, w, d, start)
			}))
		case "total_time_in_bed":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.TotalTimeInBed.IsUnset() {
					return sqlite.Arg(nil).WriteSQL(ctx, w, d, start)
				}
				return sqlite.Arg(s.TotalTimeInBed.MustGetNull()).WriteSQL(ctx, w, d, start)
			}))
		case "time_asleep":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.TimeAsleep.IsUnset() {
					return sqlite.Arg(nil).WriteSQL(ctx, w, d, start)
				}
				return sqlite.Arg(s.TimeAsleep.MustGetNull()).WriteSQL(ctx, w, d, start)
			}))
		case "origin":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.Origin.IsUnset() {
					return sqlite.Arg(nil).WriteSQL(ctx, w, d, start)
				}
				return sqlite.Arg(s.Origin.MustGet()).WriteSQL(ctx, w, d, start)
			}))
		case "raw":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.Raw.IsUnset() {
					return sqlite.Arg(nil).WriteSQL(ctx, w, d, start)
				}
				return sqlite.Arg(s.Raw.MustGetNull()).WriteSQL(ctx, w, d, start)
			}))
		case "NOTE":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.NOTE.IsUnset() {
					return sqlite.Arg(nil).WriteSQL(ctx, w, d, start)
				}
				return sqlite.Arg(s.NOTE.MustGetNull()).WriteSQL(ctx, w, d, start)
			}))
		case "title":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.Title.IsUnset() {
					return sqlite.Arg(nil).WriteSQL(ctx, w, d, start)
				}
				return sqlite.Arg(s.Title.MustGet()).WriteSQL(ctx, w, d, start)
			}))
		}
	}

	q.AppendValues(vals...)
}

func (s SleepLogSetter) UpdateMod() bob.Mod[*dialect.UpdateQuery] {
	return um.Set(s.Expressions()...)
}

func (s SleepLogSetter) Expressions(prefix ...string) []bob.Expression {
	exprs := make([]bob.Expression, 0, 11)

	if s.ID.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "id")...),
			sqlite.Arg(s.ID),
		}})
	}

	if s.DayID.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "day_id")...),
			sqlite.Arg(s.DayID),
		}})
	}

	if s.StartTime.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "start_time")...),
			sqlite.Arg(s.StartTime),
		}})
	}

	if s.EndTime.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "end_time")...),
			sqlite.Arg(s.EndTime),
		}})
	}

	if s.IsMainSleep.IsValue() || s.IsMainSleep.IsNull() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "is_main_sleep")...),
			sqlite.Arg(s.IsMainSleep),
		}})
	}

	if s.TotalTimeInBed.IsValue() || s.TotalTimeInBed.IsNull() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "total_time_in_bed")...),
			sqlite.Arg(s.TotalTimeInBed),
		}})
	}

	if s.TimeAsleep.IsValue() || s.TimeAsleep.IsNull() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "time_asleep")...),
			sqlite.Arg(s.TimeAsleep),
		}})
	}

	if s.Origin.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "origin")...),
			sqlite.Arg(s.Origin),
		}})
	}

	if s.Raw.IsValue() || s.Raw.IsNull() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "raw")...),
			sqlite.Arg(s.Raw),
		}})
	}

	if s.NOTE.IsValue() || s.NOTE.IsNull() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "NOTE")...),
			sqlite.Arg(s.NOTE),
		}})
	}

	if s.Title.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "title")...),
			sqlite.Arg(s.Title),
		}})
	}

	return exprs
}

// scan.Mapper[*T] without reflection: maps columns to fields by index.
func sleepLogScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*SleepLog, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *SleepLog) any
	}
	targets := make([]target, 0, 11)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *SleepLog) any { return &o.ID }})
		case "day_id":
			targets = append(targets, target{i, func(o *SleepLog) any { return &o.DayID }})
		case "start_time":
			targets = append(targets, target{i, func(o *SleepLog) any { return &o.StartTime }})
		case "end_time":
			targets = append(targets, target{i, func(o *SleepLog) any { return &o.EndTime }})
		case "is_main_sleep":
			targets = append(targets, target{i, func(o *SleepLog) any { return &o.IsMainSleep }})
		case "total_time_in_bed":
			targets = append(targets, target{i, func(o *SleepLog) any { return &o.TotalTimeInBed }})
		case "time_asleep":
			targets = append(targets, target{i, func(o *SleepLog) any { return &o.TimeAsleep }})
		case "origin":
			targets = append(targets, target{i, func(o *SleepLog) any { return &o.Origin }})
		case "raw":
			targets = append(targets, target{i, func(o *SleepLog) any { return &o.Raw }})
		case "NOTE":
			targets = append(targets, target{i, func(o *SleepLog) any { return &o.NOTE }})
		case "title":
			targets = append(targets, target{i, func(o *SleepLog) any { return &o.Title }})
		}
	}

	return func(row *scan.Row) (any, error) {
			o := new(SleepLog)
			for _, t := range targets {
				row.ScheduleScanByIndex(t.idx, t.dst(o))
			}
			return o, nil
		}, func(v any) (*SleepLog, error) {
			return v.(*SleepLog), nil
		}
}

// FindSleepLog retrieves a single record by primary key
// If cols is empty Find will return all columns.
func FindSleepLog(ctx context.Context, exec bob.Executor, IDPK int64, cols ...string) (*SleepLog, error) {
	if len(cols) == 0 {
		return SleepLogs.Query(
			sm.Where(SleepLogs.Columns.ID.EQ(sqlite.Arg(IDPK))),
		).One(ctx, exec)
	}

	return SleepLogs.Query(
		sm.Where(SleepLogs.Columns.ID.EQ(sqlite.Arg(IDPK))),
		sm.Columns(SleepLogs.Columns.Only(cols...)),
	).One(ctx, exec)
}

// SleepLogExists checks the presence of a single record by primary key
func SleepLogExists(ctx context.Context, exec bob.Executor, IDPK int64) (bool, error) {
	return SleepLogs.Query(
		sm.Where(SleepLogs.Columns.ID.EQ(sqlite.Arg(IDPK))),
	).Exists(ctx, exec)
}

// AfterQueryHook is called after SleepLog is retrieved from the database
func (o *SleepLog) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	var err error

	switch queryType {
	case bob.QueryTypeSelect:
		ctx, err = SleepLogs.AfterSelectHooks.RunHooks(ctx, exec, SleepLogSlice{o})
	case bob.QueryTypeInsert:
		ctx, err = SleepLogs.AfterInsertHooks.RunHooks(ctx, exec, SleepLogSlice{o})
	case bob.QueryTypeUpdate:
		ctx, err = SleepLogs.AfterUpdateHooks.RunHooks(ctx, exec, SleepLogSlice{o})
	case bob.QueryTypeDelete:
		ctx, err = SleepLogs.AfterDeleteHooks.RunHooks(ctx, exec, SleepLogSlice{o})

	}

	return err
}

// primaryKeyVals returns the primary key values of the SleepLog
func (o *SleepLog) primaryKeyVals() bob.Expression {
	return sqlite.Arg(o.ID)
}

func (o *SleepLog) pkEQ() dialect.Expression {
	return sqlite.Quote("sleep_log", "id").EQ(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
		return o.primaryKeyVals().WriteSQL(ctx, w, d, start)
	}))
}

// Update uses an executor to update the SleepLog
func (o *SleepLog) Update(ctx context.Context, exec bob.Executor, s *SleepLogSetter) error {
	v, err := SleepLogs.Update(s.UpdateMod(), um.Where(o.pkEQ())).One(ctx, exec)
	if err != nil {
		return err
	}

	o.R = v.R
	*o = *v

	return nil
}

// Delete deletes a single SleepLog record with an executor
func (o *SleepLog) Delete(ctx context.Context, exec bob.Executor) error {
	_, err := SleepLogs.Delete(dm.Where(o.pkEQ())).Exec(ctx, exec)
	return err
}

// Reload refreshes the SleepLog using the executor
func (o *SleepLog) Reload(ctx context.Context, exec bob.Executor) error {
	o2, err := SleepLogs.Query(
		sm.Where(SleepLogs.Columns.ID.EQ(sqlite.Arg(o.ID))),
	).One(ctx, exec)
	if err != nil {
		return err
	}
	o2.R = o.R
	*o = *o2

	return nil
}

// AfterQueryHook is called after SleepLogSlice is retrieved from the database
func (o SleepLogSlice) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	var err error

	switch queryType {
	case bob.QueryTypeSelect:
		ctx, err = SleepLogs.AfterSelectHooks.RunHooks(ctx, exec, o)
	case bob.QueryTypeInsert:
		ctx, err = SleepLogs.AfterInsertHooks.RunHooks(ctx, exec, o)
	case bob.QueryTypeUpdate:
		ctx, err = SleepLogs.AfterUpdateHooks.RunHooks(ctx, exec, o)
	case bob.QueryTypeDelete:
		ctx, err = SleepLogs.AfterDeleteHooks.RunHooks(ctx, exec, o)

	}

	return err
}

func (o SleepLogSlice) pkIN() dialect.Expression {
	if len(o) == 0 {
		return sqlite.Raw("NULL")
	}

	return sqlite.Quote("sleep_log", "id").In(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
		pkPairs := make([]bob.Expression, len(o))
		for i, row := range o {
			pkPairs[i] = row.primaryKeyVals()
		}
		return bob.ExpressSlice(ctx, w, d, start, pkPairs, "", ", ", "")
	}))
}

// copyMatchingRows finds models in the given slice that have the same primary key
// then it first copies the existing relationships from the old model to the new model
// and then replaces the old model in the slice with the new model
func (o SleepLogSlice) copyMatchingRows(from ...*SleepLog) {
	fromByPK := make(map[int64]*SleepLog, len(from))
	for _, new := range from {
		// keep the first row for each key, like the nested loop did
		if _, ok := fromByPK[new.ID]; !ok {
			fromByPK[new.ID] = new
		}
	}

	for i, old := range o {
		new, ok := fromByPK[old.ID]
		if !ok {
			continue
		}
		new.R = old.R
		o[i] = new
	}
}

// UpdateMod modifies an update query with "WHERE primary_key IN (o...)"
func (o SleepLogSlice) UpdateMod() bob.Mod[*dialect.UpdateQuery] {
	return bob.ModFunc[*dialect.UpdateQuery](func(q *dialect.UpdateQuery) {
		q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
			return SleepLogs.BeforeUpdateHooks.RunHooks(ctx, exec, o)
		})

		q.AppendLoader(bob.LoaderFunc(func(ctx context.Context, exec bob.Executor, retrieved any) error {
			var err error
			switch retrieved := retrieved.(type) {
			case *SleepLog:
				o.copyMatchingRows(retrieved)
			case []*SleepLog:
				o.copyMatchingRows(retrieved...)
			case SleepLogSlice:
				o.copyMatchingRows(retrieved...)
			default:
				// If the retrieved value is not a SleepLog or a slice of SleepLog
				// then run the AfterUpdateHooks on the slice
				_, err = SleepLogs.AfterUpdateHooks.RunHooks(ctx, exec, o)
			}

			return err
		}))

		q.AppendWhere(o.pkIN())
	})
}

// DeleteMod modifies an delete query with "WHERE primary_key IN (o...)"
func (o SleepLogSlice) DeleteMod() bob.Mod[*dialect.DeleteQuery] {
	return bob.ModFunc[*dialect.DeleteQuery](func(q *dialect.DeleteQuery) {
		q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
			return SleepLogs.BeforeDeleteHooks.RunHooks(ctx, exec, o)
		})

		q.AppendLoader(bob.LoaderFunc(func(ctx context.Context, exec bob.Executor, retrieved any) error {
			var err error
			switch retrieved := retrieved.(type) {
			case *SleepLog:
				o.copyMatchingRows(retrieved)
			case []*SleepLog:
				o.copyMatchingRows(retrieved...)
			case SleepLogSlice:
				o.copyMatchingRows(retrieved...)
			default:
				// If the retrieved value is not a SleepLog or a slice of SleepLog
				// then run the AfterDeleteHooks on the slice
				_, err = SleepLogs.AfterDeleteHooks.RunHooks(ctx, exec, o)
			}

			return err
		}))

		q.AppendWhere(o.pkIN())
	})
}

func (o SleepLogSlice) UpdateAll(ctx context.Context, exec bob.Executor, vals SleepLogSetter) error {
	if len(o) == 0 {
		return nil
	}

	_, err := SleepLogs.Update(vals.UpdateMod(), o.UpdateMod()).All(ctx, exec)
	return err
}

func (o SleepLogSlice) DeleteAll(ctx context.Context, exec bob.Executor) error {
	if len(o) == 0 {
		return nil
	}

	_, err := SleepLogs.Delete(o.DeleteMod()).Exec(ctx, exec)
	return err
}

func (o SleepLogSlice) ReloadAll(ctx context.Context, exec bob.Executor) error {
	if len(o) == 0 {
		return nil
	}

	o2, err := SleepLogs.Query(sm.Where(o.pkIN())).All(ctx, exec)
	if err != nil {
		return err
	}

	o.copyMatchingRows(o2...)

	return nil
}

// Day starts a query for related objects on day
func (o *SleepLog) Day(mods ...bob.Mod[*dialect.SelectQuery]) DaysQuery {
	return Days.Query(append(mods,
		sm.Where(Days.Columns.ID.EQ(sqlite.Arg(o.DayID))),
	)...)
}

func (os SleepLogSlice) Day(mods ...bob.Mod[*dialect.SelectQuery]) DaysQuery {
	PKArgSlice := make([]bob.Expression, len(os))
	for i, o := range os {
		PKArgSlice[i] = sqlite.ArgGroup(o.DayID)
	}
	PKArgExpr := sqlite.Group(PKArgSlice...)

	return Days.Query(append(mods,
		sm.Where(sqlite.Group(Days.Columns.ID).OP("IN", PKArgExpr)),
	)...)
}

func attachSleepLogDay0(ctx context.Context, exec bob.Executor, count int, sleepLog0 *SleepLog, day1 *Day) (*SleepLog, error) {
	setter := &SleepLogSetter{
		DayID: omit.From(day1.ID),
	}

	err := sleepLog0.Update(ctx, exec, setter)
	if err != nil {
		return nil, fmt.Errorf("attachSleepLogDay0: %w", err)
	}

	return sleepLog0, nil
}

func (sleepLog0 *SleepLog) InsertDay(ctx context.Context, exec bob.Executor, related *DaySetter) error {
	var err error

	day1, err := Days.Insert(related).One(ctx, exec)
	if err != nil {
		return fmt.Errorf("inserting related objects: %w", err)
	}

	_, err = attachSleepLogDay0(ctx, exec, 1, sleepLog0, day1)
	if err != nil {
		return err
	}

	sleepLog0.R.Day = day1
	sleepLog0.R.Loaded.Day = true

	day1.R.SleepLogs = append(day1.R.SleepLogs, sleepLog0)

	return nil
}

func (sleepLog0 *SleepLog) AttachDay(ctx context.Context, exec bob.Executor, day1 *Day) error {
	var err error

	_, err = attachSleepLogDay0(ctx, exec, 1, sleepLog0, day1)
	if err != nil {
		return err
	}

	sleepLog0.R.Day = day1
	sleepLog0.R.Loaded.Day = true

	day1.R.SleepLogs = append(day1.R.SleepLogs, sleepLog0)

	return nil
}

type sleepLogWhere[Q sqlite.Filterable] struct {
	cols           sleepLogColumns
	ID             sqlite.WhereMod[Q, int64]
	DayID          sqlite.WhereMod[Q, int64]
	StartTime      sqlite.WhereMod[Q, time.Time]
	EndTime        sqlite.WhereMod[Q, time.Time]
	IsMainSleep    sqlite.WhereNullMod[Q, bool]
	TotalTimeInBed sqlite.WhereNullMod[Q, int64]
	TimeAsleep     sqlite.WhereNullMod[Q, int64]
	Origin         sqlite.WhereMod[Q, string]
	Raw            sqlite.WhereNullMod[Q, string]
	NOTE           sqlite.WhereNullMod[Q, string]
	Title          sqlite.WhereMod[Q, string]
	R              sleepLogWhereR[Q]
}

func (sleepLogWhere[Q]) AliasedAs(alias string) sleepLogWhere[Q] {
	return buildSleepLogWhere[Q](buildSleepLogColumns(alias))
}

func buildSleepLogWhere[Q sqlite.Filterable](cols sleepLogColumns) sleepLogWhere[Q] {
	return sleepLogWhere[Q]{
		cols:           cols,
		ID:             sqlite.Where[Q, int64](cols.ID.Expression),
		DayID:          sqlite.Where[Q, int64](cols.DayID.Expression),
		StartTime:      sqlite.Where[Q, time.Time](cols.StartTime.Expression),
		EndTime:        sqlite.Where[Q, time.Time](cols.EndTime.Expression),
		IsMainSleep:    sqlite.WhereNull[Q, bool](cols.IsMainSleep.Expression),
		TotalTimeInBed: sqlite.WhereNull[Q, int64](cols.TotalTimeInBed.Expression),
		TimeAsleep:     sqlite.WhereNull[Q, int64](cols.TimeAsleep.Expression),
		Origin:         sqlite.Where[Q, string](cols.Origin.Expression),
		Raw:            sqlite.WhereNull[Q, string](cols.Raw.Expression),
		NOTE:           sqlite.WhereNull[Q, string](cols.NOTE.Expression),
		Title:          sqlite.Where[Q, string](cols.Title.Expression),
		R:              sleepLogWhereR[Q]{cols: cols},
	}
}

// sleepLogWhereR holds the relationship-based filters of
// sleepLogWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type sleepLogWhereR[Q sqlite.Filterable] struct {
	cols sleepLogColumns
}

// HasDay filters parents that have a matching Day using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to Days).
func (w sleepLogWhereR[Q]) HasDay(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(Days.NameExpr()),
		sm.Where(Days.Columns.ID.EQ(w.cols.DayID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

func (o *SleepLog) Preload(name string, retrieved any) error {
	if o == nil {
		return nil
	}

	switch name {
	case "Day":
		rel, ok := retrieved.(*Day)
		if !ok {
			return fmt.Errorf("sleepLog cannot load %T as %q", retrieved, name)
		}

		o.R.Day = rel
		o.R.Loaded.Day = true

		if rel != nil {
			rel.R.SleepLogs = SleepLogSlice{o}
		}
		return nil
	default:
		return fmt.Errorf("sleepLog has no relationship %q", name)
	}
}

type sleepLogPreloader struct {
	Day func(...sqlite.PreloadOption) sqlite.Preloader
}

func buildSleepLogPreloader() sleepLogPreloader {
	return sleepLogPreloader{
		Day: func(opts ...sqlite.PreloadOption) sqlite.Preloader {
			return sqlite.Preload[*Day, DaySlice](sqlite.PreloadRel{
				Name: "Day",
				Sides: []sqlite.PreloadSide{
					{
						From:        SleepLogs,
						To:          Days,
						FromColumns: []string{"day_id"},
						ToColumns:   []string{"id"},
					},
				},
			}, Days.Columns.Names(), dayScanMapperNullable, opts...)
		},
	}
}

type sleepLogThenLoader[Q orm.Loadable] struct {
	Day func(...bob.Mod[*dialect.SelectQuery]) orm.Loader[Q]
}

func buildSleepLogThenLoader[Q orm.Loadable]() sleepLogThenLoader[Q] {
	type DayLoadInterface interface {
		LoadDay(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
	}

	return sleepLogThenLoader[Q]{
		Day: thenLoadBuilder[Q](
			"Day",
			func(ctx context.Context, exec bob.Executor, retrieved DayLoadInterface, mods ...bob.Mod[*dialect.SelectQuery]) error {
				return retrieved.LoadDay(ctx, exec, mods...)
			},
		),
	}
}

// LoadDay loads the sleepLog's Day into the .R struct
func (o *SleepLog) LoadDay(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}

	// Reset the relationship
	o.R.Day = nil
	o.R.Loaded.Day = false

	related, err := o.Day(mods...).One(ctx, exec)
	if err != nil {
		return err
	}

	related.R.SleepLogs = SleepLogSlice{o}

	o.R.Day = related
	o.R.Loaded.Day = true
	return nil
}

// LoadDay loads the sleepLog's Day into the .R struct
func (os SleepLogSlice) LoadDay(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}

	days, err := os.Day(mods...).All(ctx, exec)
	if err != nil {
		return err
	}

	for _, o := range os {
		if o == nil {
			continue
		}

		o.R.Day = nil
		o.R.Loaded.Day = true
	}
	// O(N+M) stitch via a map keyed by the join column (key -> []parent; was O(N*M)).
	sleepLogByKey := make(map[int64][]*SleepLog, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		sleepLogByKey[o.DayID] = append(sleepLogByKey[o.DayID], o)
	}

	for _, rel := range days {

		owners, ok := sleepLogByKey[rel.ID]
		if !ok {
			continue
		}

		for _, o := range owners {

			// to-one: keep only the first matching child (matches the previous break)
			if o.R.Day != nil {
				continue
			}

			rel.R.SleepLogs = append(rel.R.SleepLogs, o)

			o.R.Day = rel

		}
	}

	return nil
}

type sleepLogJoins[Q dialect.Joinable] struct {
	typ string
	Day modAs[Q, dayColumns]
}

func (j sleepLogJoins[Q]) aliasedAs(alias string) sleepLogJoins[Q] {
	return buildSleepLogJoins[Q](buildSleepLogColumns(alias), j.typ)
}

func buildSleepLogJoins[Q dialect.Joinable](cols sleepLogColumns, typ string) sleepLogJoins[Q] {
	return sleepLogJoins[Q]{
		typ: typ,
		Day: modAs[Q, dayColumns]{
			c: Days.Columns,
			f: func(to dayColumns) bob.Mod[Q] {
				mods := make(mods.QueryMods[Q], 0, 1)

				{
					mods = append(mods, dialect.Join[Q](typ, Days.NameExpr().As(to.Alias())).On(
						to.ID.EQ(cols.DayID),
					))
				}

				return mods
			},
		},
	}
}

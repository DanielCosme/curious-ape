	...
	for _, col := range q.TableRef.Columns {
		...
		case "password":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				...
				return sqlite.Arg(s.Password.MustGet()).WriteSQL(ctx, w, d, start)
			}))
		case "role":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.Role.IsUnset() {
				...
				return sqlite.Arg(s.Role.MustGet()).WriteSQL(ctx, w, d, start)
			}))
		case "email":
			vals = append(vals, bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
				if s.Email.IsUnset() {
...
func userScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*User, error)) {
	...
	for i, col := range cols {
		...
		case "password":
			targets = append(targets, target{i, func(o *User) any { return &o.Password }})
		case "role":
			targets = append(targets, target{i, func(o *User) any { return &o.Role }})
		case "email":
...
func (o UserSlice) copyMatchingRows(from ...*User) {
	fromByPK := make(map[int64]*User, len(from))
...
func buildUserWhere[Q sqlite.Filterable](cols userColumns) userWhere[Q] {
	return userWhere[Q]{
		...
		Password: sqlite.Where[Q, string](cols.Password.Expression),
		Role:     sqlite.Where[Q, string](cols.Role.Expression),

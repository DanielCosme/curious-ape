...
var HabitCategories = sqlite.NewTablex[*HabitCategory, HabitCategorySlice, *HabitCategorySetter]("", "habit_category", buildHabitCategoryColumns("habit_category"), habitCategoryScanMapper)
...
func habitCategoryScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*HabitCategory, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *HabitCategory) any
	}
	targets := make([]target, 0, 4)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *HabitCategory) any { return &o.ID }})
		case "name":
			targets = append(targets, target{i, func(o *HabitCategory) any { return &o.Name }})
		case "kind":
			targets = append(targets, target{i, func(o *HabitCategory) any { return &o.Kind }})
		case "description":
			targets = append(targets, target{i, func(o *HabitCategory) any { return &o.Description }})
		}
	}
	...
	return func(row *scan.Row) (any, error) {
			...
			return o, nil
		}, func(v any) (*HabitCategory, error) {
			return v.(*HabitCategory), nil
		}
}
...
func (o HabitCategorySlice) copyMatchingRows(from ...*HabitCategory) {
	fromByPK := make(map[int64]*HabitCategory, len(from))
...
func buildHabitCategoryWhere[Q sqlite.Filterable](cols habitCategoryColumns) habitCategoryWhere[Q] {
	return habitCategoryWhere[Q]{
		...
		R:           habitCategoryWhereR[Q]{cols: cols},
	}
}

// habitCategoryWhereR holds the relationship-based filters of
// habitCategoryWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type habitCategoryWhereR[Q sqlite.Filterable] struct {
	cols habitCategoryColumns
}
...
func (w habitCategoryWhereR[Q]) HasHabits(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(Habits.NameExpr()),
		sm.Where(Habits.Columns.HabitCategoryID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// NULL-tolerant scan buffer for habitCategoryScanMapperNullable:
...
type habitCategoryPreloadBuf struct {
	ID          null.Val[int64]
	Name        null.Val[string]
	Kind        null.Val[string]
	Description null.Val[string]
}

// habitCategoryScanMapperNullable maps the preloaded habitCategory
...
func habitCategoryScanMapperNullable(prefix string) scan.Mapper[*HabitCategory] {
	return func(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*HabitCategory, error)) {
		// resolve the column names once per query, not per row
		type target struct {
			idx int
			dst func(b *habitCategoryPreloadBuf) any
		}
		targets := make([]target, 0, 4)
		for i, col := range cols {
			...
			switch name {
			case "id":
				targets = append(targets, target{i, func(b *habitCategoryPreloadBuf) any { return &b.ID }})
			case "name":
				targets = append(targets, target{i, func(b *habitCategoryPreloadBuf) any { return &b.Name }})
			case "kind":
				targets = append(targets, target{i, func(b *habitCategoryPreloadBuf) any { return &b.Kind }})
			case "description":
				targets = append(targets, target{i, func(b *habitCategoryPreloadBuf) any { return &b.Description }})
			}
		}
		...
		buf := new(habitCategoryPreloadBuf)
		return func(row *scan.Row) (any, error) {
				for _, t := range targets {
					row.ScheduleScanByIndex(t.idx, t.dst(buf))
				}
				return buf, nil
			}, func(link any) (*HabitCategory, error) {
				buf := link.(*habitCategoryPreloadBuf)

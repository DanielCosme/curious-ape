...
func upsert(db bob.Executor, params core.DeepWorkLog) (log core.DeepWorkLog, err error) {
	day, err := persistence.GetDay(db, params.Date)
	if err != nil {
		return log, persistence.CatchDBErr("fitness logs: upsert: get day", err)
	}
	...
	bobLog, err := models.DeepWorkLogs.Insert(setter).One(context.Background(), db)
	if err != nil {
		if dberrors.DeepWorkLogErrors.ErrUniqueSqliteAutoindexDeepWorkLog1.Is(err) {
			bobLog, err = get(db, core.DeepWorkLogParams{
				DayID:     day.ID,
			...
			err = bobLog.Update(context.Background(), db, setter)
		} else {
			return
		}
	}

	return deepWorkLogToCore(day, bobLog), persistence.CatchDBErr("work: upsert", err)
}

func get(db bob.Executor, p core.DeepWorkLogParams) (*models.DeepWorkLog, error) {
	workLog, err := BuildQuery(p).One(context.Background(), db)
	if err != nil {
		return nil, persistence.CatchDBErr("work logs: get", err)
	}
	return workLog, nil
}

...
func upsert(db bob.DB, params core.SleepLog) (sl core.SleepLog, err error) {
	day, err := persistence.GetDay(db, params.Date)
	if err != nil {
		return sl, persistence.CatchDBErr("sleep logs: upsert: get day", err)
	}
	...
	sleepLog, err := models.SleepLogs.Insert(s).One(context.Background(), db)
	if err != nil {
		if dberrors.SleepLogErrors.ErrUniqueSqliteAutoindexSleepLog1.Is(err) {
			ref := s.IsMainSleep.GetOrZero()
			sleepLog, err = get(db, SleepLogParams{
			...
			err = sleepLog.Update(context.Background(), db, s)
			if err != nil {
				return
			}
		} else {
	...
	return sleepLogToCore(day, sleepLog), persistence.CatchDBErr("sleep: upsert", err)
}
...
func get(db bob.DB, p SleepLogParams) (*models.SleepLog, error) {
	sleepLog, err := p.buildQuery().One(context.Background(), db)
	if err != nil {
		return nil, persistence.CatchDBErr("sleep logs: get", err)
	}
	return sleepLog, nil
}

...
type SleepLog struct {
	...
	MinutesInBed  int64     `db:"minutes_in_bed" `
	MinutesAsleep int64     `db:"minutes_asleep" `
...
func buildSleepLogColumns(alias string) sleepLogColumns {
	return sleepLogColumns{
		ColumnsExpr: expr.NewColumnsExpr(
			"id", "day_id", "title", "date", "start_time", "end_time", "note", "is_main_sleep", "minutes_in_bed", "minutes_asleep", "origin", "raw",
		...
		MinutesInBed:  sqlite.Quote(alias, "minutes_in_bed"),
...
type sleepLogColumns struct {
	...
	MinutesInBed  sqlite.Expression
...
type SleepLogSetter struct {
	...
	IsMainSleep   omit.Val[bool]      `db:"is_main_sleep" `
	MinutesInBed  omit.Val[int64]     `db:"minutes_in_bed" `
	MinutesAsleep omit.Val[int64]     `db:"minutes_asleep" `
...
func (s SleepLogSetter) SetColumns() []string {
	...
	if s.IsMainSleep.IsValue() {
		vals = append(vals, "is_main_sleep")
	}
	if s.MinutesInBed.IsValue() {
		vals = append(vals, "minutes_in_bed")
	}
	if s.MinutesAsleep.IsValue() {
...
func (s SleepLogSetter) Overwrite(t *SleepLog) {
	...
	if s.IsMainSleep.IsValue() {
		t.IsMainSleep = s.IsMainSleep.MustGet()
	}
	if s.MinutesInBed.IsValue() {
		t.MinutesInBed = s.MinutesInBed.MustGet()
	}
	if s.MinutesAsleep.IsValue() {
		t.MinutesAsleep = s.MinutesAsleep.MustGet()
	}
	if s.Origin.IsValue() {
...
func (s *SleepLogSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if s.IsMainSleep.IsValue() {
			vals = append(vals, sqlite.Arg(s.IsMainSleep.MustGet()))
		}

		if s.MinutesInBed.IsValue() {
			vals = append(vals, sqlite.Arg(s.MinutesInBed.MustGet()))
		}

		if s.MinutesAsleep.IsValue() {
			vals = append(vals, sqlite.Arg(s.MinutesAsleep.MustGet()))
		}
...
func (s SleepLogSetter) Expressions(prefix ...string) []bob.Expression {
	...
	if s.MinutesInBed.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			sqlite.Quote(append(prefix, "minutes_in_bed")...),
			sqlite.Arg(s.MinutesInBed),
		}})
	}
...
type sleepLogWhere[Q sqlite.Filterable] struct {
	...
	IsMainSleep   sqlite.WhereMod[Q, bool]
	MinutesInBed  sqlite.WhereMod[Q, int64]
	MinutesAsleep sqlite.WhereMod[Q, int64]
...
func buildSleepLogWhere[Q sqlite.Filterable](cols sleepLogColumns) sleepLogWhere[Q] {
	return sleepLogWhere[Q]{
		...
		IsMainSleep:   sqlite.Where[Q, bool](cols.IsMainSleep),
		MinutesInBed:  sqlite.Where[Q, int64](cols.MinutesInBed),
		MinutesAsleep: sqlite.Where[Q, int64](cols.MinutesAsleep),
